...
import com.tangramfactory.smartweight.utility.Utils;
...
//        paint.setStrokeCap(Paint.Cap.ROUND);
    ...
    public void setProgress(int progress) {
        ...
        if (this.progress > getMax()) {
            ...
            unfinishedStrokeColor = Color.rgb(72, 106, 176);
            firstAngleStoped = false;
            stopProgress = 0;
        }
        invalidate();
    }
    ...
    public void setUnfinishedStrokeColor(int color) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(firstAngleStoped) {
            float stopSweepAngle = stopProgress / (float) getMax() * arcAngle;
            drawArcLine(canvas, finishedStartAngle, stopSweepAngle);
        }

        drawArcLine(canvas, finishedStartAngle, finishedSweepAngle);
    ...
    private void drawArcLine(Canvas canvas, float finishedStartAngle, float finishedSweepAngle) {
        float angle = (float) ((finishedStartAngle + finishedSweepAngle + 90) * Math.PI / 180f);
        float scaleMarkSize = getResources().getDisplayMetrics().density * 60; // 16dp
        float radius = getWidth() / 2f;

        float startX = (float) (getWidth() / 2.0f + radius * Math.sin(angle));
        float startY = (float) ( getHeight() / 2.0f - radius * Math.cos(angle));

        float stopX = (float) (getWidth() / 2.0f + (radius - scaleMarkSize) * Math.sin(angle));
        float stopY = (float) ( getHeight() / 2.0f - (radius - scaleMarkSize) * Math.cos(angle));

        Log.d("LOG", "angle = " + angle);
        String msg = "startX = " + startX + ",startY = " + startY + "stopX = " + stopX + "stopY = " + stopY;
        Log.d("LOG", msg);

//        msg = "unfinishedOuterRect.left = " + unfinishedOuterRect.left + ", unfinishedOuterRect.top = " +unfinishedOuterRect.top +
//                ", unfinishedOuterRect.right = " + unfinishedOuterRect.right + ", unfinishedOuterRect.bottom = " + unfinishedOuterRect.bottom;
//        Log.d("LOG", msg);
    ...
    public void setFirstAngleStoped(boolean stop) {
        if(progress < 50) {
            firstAngleStoped = stop;
